...
- **The fill handle** must extend both *values* (copy) and *series* (detect and continue numeric/date patterns). Series detection is where most implementations feel wrong.
...
- [ ] Bean `rangeSvc` implementing `IRangeService`; `iCellRangeFeature` per-cell integration
- [ ] Single range via drag; multi-range via Ctrl/Cmd+drag
- [ ] Keyboard range extension (Shift+arrows, Shift+click)
- [ ] Range handle (drag the corner to resize the range)
- [ ] **Fill handle** — copy values, and detect/continue numeric and date series
- [ ] Options: `cellSelection` (`boolean | CellSelectionOptions`), `suppressMultiRanges`, `enableHeaderHighlight`, `enableColumnSelection`, `handle` (`RangeHandleOptions | FillHandleOptions`)
- [ ] Events: `cellSelectionDeleteStart`, `cellSelectionDeleteEnd`
- [ ] Delete/Backspace clears the selected range

### 4B — `@libregrid/clipboard`

- [ ] Bean `clipboardSvc` implementing `iClipboardService`
- [ ] Copy / cut / paste with Excel-compatible TSV
- [ ] Options: `copyHeadersToClipboard`, `suppressCutToClipboard`, `suppressClipboardPaste`, `clipboardDelimiter`, `enableCellTextSelection`, `ensureDomOrder`, `readOnlyEdit`
- [ ] Callbacks: `sendToClipboard`, `processCellForClipboard`, `processHeaderForClipboard`, `processGroupHeaderForClipboard`, `processCellFromClipboard`, `processDataFromClipboard`
- [ ] ColDef `suppressPaste` (boolean or function)
- [ ] API: `copySelectedRowsToClipboard`, `copySelectedRangeToClipboard`
- [ ] Events: `cutStart`, `cutEnd`, `pasteStart`, `pasteEnd`; `cellValueChanged` / `cellEditRequest` integration
- [ ] Contribute `copy`, `copyWithHeaders`, `copyWithGroupHeaders`, `cut`, `paste` to the Phase 1 menu registry

### 4C — `@libregrid/status-bar`

- [ ] Bean `statusBarSvc`; `iStatusPanel` contract
- [ ] Option `statusBar.statusPanels`; `StatusPanelDef`: `statusPanel`, `align`, `key`, `statusPanelParams`
- [ ] Provided panels: `agTotalRowCountComponent`, `agTotalAndFilteredRowCountComponent`, `agFilteredRowCountComponent`, `agSelectedRowCountComponent`, `agAggregationComponent`
- [ ] API `getStatusPanel(key)`
- [ ] Custom panel contract: `agInit(params)` required, `refresh(params)` optional
- [ ] Material implementation (`MatToolbar`)
