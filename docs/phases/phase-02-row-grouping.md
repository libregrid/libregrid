...
- [x] `refreshAfterGroupEdit` — **shipped in Phase 21** (gap-plan A8, `RowGroupingEditModule`). The original rationale here ("LibreGrid has no cell-editing feature yet in any package") was stale — cell editing is Community stock — and the option is validated against `RowGrouping`/`TreeData`, not `RowGroupingEdit`. See [`phase-21-row-grouping-edit.md`](phase-21-row-grouping-edit.md)
