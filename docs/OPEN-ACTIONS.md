...
| #   | Action                                | Status                     | Notes                                                                                                                                                                                                                                                                                                                                                               |
| --- | ------------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A8  | **Publish 1.0.0** with npm provenance | ✅ **done 2026-08-15**     | All 26 `@libregrid/*` packages published at 1.0.0 on 2026-08-15, superseded by 1.1.0 on 2026-08-17 and 1.1.1 on 2026-08-18. npm provenance attestations confirmed (SLSA v1). The verbatim quick-start check (Phase 13 acceptance) remains externally owned. |
| A9  | Phase 4 external spreadsheet check    | ✅ **done 2026-08-17**     | Copy-paste from the grid verified in Excel, LibreOffice Calc, and Google Sheets — row/column shape confirmed correct. Phase 4 is now complete. |
...
| B5  | Public AI demo cost and abuse control | ⬜ open | The Cloud Run demo gateway holds a real provider key. Turnstile guards it and `--max-instances 3` caps burst. The runtime uses a dedicated least-privilege service account (`libregrid-ai-gateway@libregrid.iam.gserviceaccount.com`), not the default compute service account. The default compute service account still holds `roles/editor` project-wide. Reviewing and tightening that grant is an open item for the project owner. Set a Google Cloud billing budget alert, and review OpenRouter usage weekly for the first month. |
...
| C3  | Context-menu submenu expansion                    | ✅ **done 2026-08-16** | The `@libregrid/menu` renderer now opens submenus on hover/click/ArrowRight: registry items with string `subMenu` entries (e.g. Export → csvExport/excelExport) are resolved recursively, submenus render inside the popup element so outside-click detection keeps them open, ArrowLeft returns focus to the parent, Escape closes everything, and separators span the full menu width. |
