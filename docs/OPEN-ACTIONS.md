...
| C3  | Context-menu submenu expansion                    | ⬜ open                | Found during Phase 5.6: the Phase 1 menu renderer shows a submenu arrow and `aria-haspopup` but never opens nested menus (Export → CSV/Excel). The Excel Export contribution registers correctly; expanding the submenu is a `@libregrid/menu` follow-up, tracked here.                                                                                                                                        |
