...
| `refreshAfterGroupEdit` | ✅ | Phase 21 (A8) — when `true`, an edit to an active row-group column re-evaluates the hierarchy (`refreshModel({ step: 'group', rowDataUpdated: true })`), moving the row to the correct group while preserving expansion by deterministic ID. Community's `RowDragService` already read this option for managed drag-handle visibility, so the flag also starts enabling managed drag-between-groups (A11). Tree data also honours it (any edit may re-path). Integration-tested |
...
| `showValuesAsDef` | ✅ | PR 2.5 — `precision`/`suppressHeaderIndicator` read; Phase 14 (A10) — `modes` (custom mode registry / built-in overrides) implemented, see "Show Values As" below |
| `enableShowValuesAs` | ✅ | PR 2.5 — read by `isMenuEligible`; Phase 14 (A10) — the eligible menu is wired: the service registers a `showValuesAs` column-menu contribution at `postConstruct` (see "Column-menu integration" below) |
| `groupRowEditable` | ✅ | Phase 21 (A8) — `boolean \| GroupRowEditableCallback`. The only option that opens the edit entry point: Community's `isCellEditable` routes a group row through `rowGroupingEditValueSvc.isGroupCellEditable`, which evaluates the flag/callback **and** resolves the distribution (a suppressed distribution makes the cell non-editable even when the flag is `true`). Columns with this option need no `field`/`valueSetter`. Integration-tested (flag, callback, and the `groupRowValueSetter: false` override) |
| `groupRowValueSetter` | ✅ | Phase 21 (A8) — `boolean \| GroupRowValueSetterFunc \| GroupRowValueSetterOptions`. Governs distribution for every `setDataValue` on a group row (also API/undo/paste writes). Absent → the seam returns `undefined` so the caller keeps the normal path; `false` suppresses; a function receives `GroupRowValueSetterParams`; an options object resolves `distribution` (strategy / `true` / `false` / per-aggFunc record), `default`, `precision`, and `getValue`/`setValue`. An explicit per-aggFunc entry of `true` enables `count`/`min`/`max`/`first`/`last` (`'overwrite'`); a top-level `distribution: true` still suppresses them; and `default` never enables those five — they must be listed explicitly (matches ag-grid.com). Unit- and integration-tested |
...
| `percentOfColumnTotal` | ✅ | Identical to `percentOfGrandTotal` — correct, not a gap: the interface's own doc says `columnTotal()` "equal to `grandTotal()` unless pivoting", and this service does not compute pivot-axis totals (the Phase 8 pivot package is out of scope for the showValuesAs service) |
...
| `percentOfParentColumnTotal` | ❌ | Never applies — `parentColumnTotal()` is documented as "null when not pivoting", and this service does not compute pivot-axis totals (the Phase 8 pivot package is out of scope for the showValuesAs service). The mode's `applicability` returns `'hide'` (omitted from the menu) when not pivoting and `'inapplicable'` when pivoting; `isApplying` returns `false` either way, so cells show the raw value rather than a broken transform; integration-tested |
| `showValuesAsDef.modes` (custom modes / built-in overrides) | ✅ | Phase 14 (A10) — `buildResolvedModes` starts from the five built-ins and applies per-column `modes` entries: `false`/`null` hides a built-in, `true` keeps it, a function receives the base def and returns a partial, and an object deep-merges over the built-in def (per the built-in `modes` docs: "deep-merged with the built-in mode definitions"). Unknown names become custom modes. `showValuesAsDef: null` on a column suppresses the grid-wide def. Unit-tested (deep merge, function entry, `defaultColDef` merge) |
| `ShowValuesAsModeDef.menu` (per-mode custom submenu builder) | ❌ | Not implemented — per-mode custom submenu builders are not provided; this is the documented extension point for per-mode params dialogs (e.g. "% Of <column>" pickers). The column menu offers a flat "None + one entry per available mode" submenu instead; `getMenuItems` is unit-tested as the seam for a future per-mode builder |
| Column-menu integration | ✅ | Phase 14 (A10) — the service registers a `showValuesAs` column-menu contribution at `postConstruct` via the `menuItemMapper` bean's `registry.register` (module-scope registration like PR 2.5's `registerMenuItems` is not possible here: the factory needs the service instance + `gos`, and the global store is only read in the registry constructor). The "Show Values As" submenu lists `None` plus each available mode; selecting an entry calls `api.applyColumnState({ state: [{ colId, showValuesAs }] })`, which Community's `syncColumnState` routes back into `syncColState` → `applySelection`. `percentOfParentColumnTotal` is hidden outside pivot; a hidden mode is kept in the menu (checked + disabled) when it is the active selection. When `@libregrid/menu` is not registered the registration silently skips. Integration-tested: menu open → submenu → select → `getCellValue({ useFormatter: true, transformValues: true })` returns the percentage; `None` clears |

## Editing Groups (Phase 21, gap-plan A8)

`RowGroupingEditModule` (`moduleName: 'RowGroupingEdit'`) is a separate module, matching the
site's `RowGroupingEditModule`. Community validates `colDef.groupRowEditable` /
`colDef.groupRowValueSetter` against the `RowGroupingEdit` name and `refreshAfterGroupEdit`
against `RowGrouping`/`TreeData`, so **both** names must be registered for a clean grid.

| Item | Status | Notes |
|---|---|---|
| `rowGroupingEditValueSvc` bean | ✅ | `RowGroupingEditService` — fills the bean slot Community declares at `context.d.ts:275` but never provides. `setGroupDataValue` returns `undefined` when neither colDef option is set (caller keeps the normal path), `false` when distribution is suppressed, else the boolean "children changed" result. Integration-tested |
| `aggChildrenSvc` bean | ✅ | `AggregatedChildrenService` — the second unfilled slot (`context.d.ts:264`). Backs the public `rowNode.getAggregatedChildren()`, which returned `[]` unconditionally before Phase 21. Immediate children for a group, all descendant leaves when `recursive`, `[]` for a leaf node, pivot-key filtering for a pivot result column, and nothing outside the CSRM. With `suppressAggFilteredOnly` the returned set spans every child, matching the aggregation scope rather than the filter (matches ag-grid.com). Integration-tested |
| `distributeGroupValue(params, options?)` | ✅ | Exported from `@libregrid/row-grouping`. Documented upstream as an `ag-grid-enterprise` export that does not exist in the community dist, so it is built from the public `GroupRowValueSetter*` types under guardrail G2. Strategies `'uniform'`, `'percentage'` (zero-total → `'uniform'`), `'increment'` (delta split for `sum`, full delta for `avg`), `'overwrite'`; `sum`-vs-`avg` arithmetic; `getValue`/`setValue` overrides; per-aggFunc records with `default` fallback |
| Distribution defaults by aggFunc | ✅ | No aggFunc → `'overwrite'`; `sum` → `'uniform'`; `avg` → `'overwrite'`; `count`/`min`/`max`/`first`/`last` and custom aggFuncs disabled unless enabled by an explicit record entry or `distribution: true` (which gives custom aggFuncs `'overwrite'` but still not the disabled set). Unit-tested as a matrix |
| `precision` rounding | ✅ | Explicit `number` / `false`; auto-detect from `cellEditorParams.precision`, then a whole-number `cellEditorParams.step` → `0`. The rounding remainder is spread across children so the child total matches the target exactly (`10 / 3` at `precision: 0` → `4, 3, 3`; at `2` → `3.34, 3.33, 3.33`). `bigint` values always distribute as integers. Unit-tested |
| Group-cell editability | ✅ | Verified end-to-end with `api.startEditingCell` on a group row: editable with `groupRowEditable: true`/callback, not editable without it, and not editable when `groupRowValueSetter` resolves to `false` |
| Same-pass re-aggregation | ✅ | `changedPathFactory` tracks edited rows and their ancestors so Community batches aggregation and refreshes visible group cells after both leaf and group edits. Nested distribution performs one aggregate pass; the manual re-aggregation workaround has been removed. Integration and browser regression coverage |
| `aggregateOnlyChangedColumns` (interaction) | 🟡 | Change detection now receives a row path, but column-level optimisation remains unimplemented: `AggregationStage` re-traverses the whole tree and recomputes all value columns |
