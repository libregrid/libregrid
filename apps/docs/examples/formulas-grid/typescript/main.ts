...
const discountStore = new Map<string, string>([
...
const gridOptions: GridOptions<Row> = {
  ...
  formulaDataSource: {
    getFormula: ({ column, rowNode }) => discountStore.get(`${rowNode.id}-${column.getColId()}`),
    setFormula: ({ column, rowNode, formula }) => {
      const key = `${rowNode.id}-${column.getColId()}`;
      if (formula === undefined) discountStore.delete(key);
      else discountStore.set(key, formula);
    },
  },
  formulaFuncs: {
    MARKUP: {
