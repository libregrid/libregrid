...
@Component({
  ...
  styles: `
    ...
    pre { margin: .75rem 0 0; overflow: auto; padding: 1rem; border-radius: var(--lgr-radius-md); background: var(--mat-sys-surface-container); border: 1px solid var(--mat-sys-outline-variant); line-height: 1.5; }
    ...
    <div class="lgr-page">
      ...
      <div class="steps">
        <mat-card appearance="outlined" class="step">
          <span class="step-number">1</span><div><h2>Install only the capability you need</h2><p>Start focused; add packages as your product requires more. Copy the install command from the <a routerLink="/">overview page</a>.</p></div>
        </mat-card>
        <mat-card appearance="outlined" class="step">
      ...
      <mat-card appearance="outlined"><mat-card-content><h2>Where your backend belongs</h2><p>Client-side features need only rows. Server-side features hand your existing API a typed request containing range, sorting, grouping and filters; your service applies it to the database and returns rows plus a total count. Visit Server-Side Analytics for the complete request and response contract.</p></mat-card-content></mat-card>
