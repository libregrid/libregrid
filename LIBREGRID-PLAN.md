...
3. **The core is pre-wired for the plug-ins.** `iModule.ts` declares every Enterprise module name; `context.ts` reserves DI bean slots for them under the comment *"Things used in enterprise or elsewhere that we haven't created interfaces for."*
...
- [ ] **Phase 3 — Columns tool panel** · [`phases/phase-03-columns-tool-panel.md`](docs/phases/phase-03-columns-tool-panel.md)
  Column chooser, row-group/value/pivot drop zones, CDK drag-drop.
...
| `@libregrid/columns-tool-panel` | 3 | `ColumnsToolPanel` |
