...
export function buildGridTheme(
...
function resolveTypography(root: HTMLElement): { fontFamily: string; fontSize: number } {
  const document = root.ownerDocument;
  const probe = document.createElement('span');
  probe.style.cssText = 'position:absolute;visibility:hidden;pointer-events:none;font:var(--mat-sys-body-medium,400 14px/20px Roboto,system-ui,sans-serif)';
  root.appendChild(probe);
