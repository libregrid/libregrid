...
import type { AgColumn, AgShowValuesAsResolved, IRowNode, RowNode, ShowValuesAsType } from 'ag-grid-community';
...
function makeColumn(opts: FakeColumnOptions = {}) {
...
interface HarnessBeans {
  ...
  gridApi?: { refreshCells: ReturnType<typeof vi.fn> };
...
describe('ShowValuesAsService', () => {
  describe('resolveColumn', () => {
    ...
    it('defaults precision to 2 and suppressHeaderIndicator to undefined when rawDef is absent', () => {
      const { bean, destroy } = harness();
      const col = makeColumn({ colDef: {} });
      bean.resolveColumn(col, true);
      expect(col.showValuesAsDef).toEqual({ modes: {}, precision: 2, suppressHeaderIndicator: undefined });
      expect(col.showValuesAs).toBeNull();
      destroy();
    });
  ...
  describe('transform', () => {
    ...
    it('percentOfParentColumnTotal always computes null', () => {
      const { bean, destroy } = harness();
      const col = makeColumn({ showValuesAs: resolved('percentOfParentColumnTotal') });
      expect(bean.transform(col, {} as IRowNode, 100)).toBeNull();
      destroy();
    });
  ...
  describe('formatValue', () => {
    it('returns #N/A when notApplicable', () => {
      const { bean, destroy } = harness();
      expect(bean.formatValue(makeColumn(), null, 12, 12, true)).toBe('#N/A');
      destroy();
    });

    it('returns null when the transformed value is null', () => {
      const { bean, destroy } = harness();
      expect(bean.formatValue(makeColumn(), null, null, 12, false)).toBeNull();
      destroy();
    });
    ...
    it('uses resolved precision, then showValuesAsDef precision, then 2', () => {
      ...
      const fromDef = makeColumn({ showValuesAsDef: { precision: 4 } });
      expect(bean.formatValue(fromDef, null, 12.345678, 1, false)).toBe('12.3457%');

      expect(bean.formatValue(makeColumn(), null, 12.345, 1, false)).toBe('12.35%');
      destroy();
    });
  });

  describe('getActiveModeLabel / getActiveModeTooltip', () => {
    it('getActiveModeLabel falls back to the raw type for unknown modes', () => {
      const { bean, destroy } = harness();
      const col = makeColumn({ showValuesAs: resolved('bogusMode') });
      expect(bean.getActiveModeLabel(col)).toBe('bogusMode');
      destroy();
    });
  ...
  describe('isMenuEligible', () => {
    it('requires colDef.enableShowValuesAs === true', () => {
      const { bean, destroy } = harness();
  ...
  describe('getMenuItems', () => {
    it('offers None plus one item per built-in mode, checking the active one', () => {
      const { bean, destroy } = harness();
